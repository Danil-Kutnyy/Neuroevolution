...
class L29(tf.keras.layers.Layer):
	...
	def call(self, inputs):
		return tf.keras.activations.softmax(tf.math.abs(inputs))
...
class L1005(tf.keras.layers.Layer):
	def __init__(self):
		super(L1005, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1006(tf.keras.layers.Layer):
	def __init__(self):
		super(L1006, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1007(tf.keras.layers.Layer):
	def __init__(self):
		super(L1007, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1008(tf.keras.layers.Layer):
	def __init__(self):
		super(L1008, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1009(tf.keras.layers.Layer):
	def __init__(self):
		super(L1009, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1010(tf.keras.layers.Layer):
	def __init__(self):
		super(L1010, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1011(tf.keras.layers.Layer):
	def __init__(self):
		super(L1011, self).__init__()
	def build(self, input_shape):
		self.i_layer = tf.keras.layers.Dense( units=2 )
	def call(self, inputs):
		return self.i_layer(inputs)
	
class L1012(tf.keras.layers.Layer):
	def __init__(self):
		super(L1012, self).__init__()
	def call(self, inputs):
		return inputs
...
layer_113 = L113()
layer_113_out = layer_113(layer_1_out)

layer_114 = L114()
try:
	try:
		layer_114_concat = tf.keras.layers.concatenate([layer_1_out, layer_113_out])
		layer_114_out = layer_114(layer_114_concat)
	except ValueError:
		layer_114_concat = tf.keras.layers.concatenate([layer_1_out, layer_113_out], axis=-2)
		layer_114_out = layer_114(layer_114_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_114 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_113_out]:
		if type(i) == tuple:
			for i_2 in i:
...
try:
	try:
		layer_112_concat = tf.keras.layers.concatenate([layer_1_out, layer_113_out, layer_114_out])
		layer_112_out = layer_112(layer_112_concat)
	except ValueError:
		layer_112_concat = tf.keras.layers.concatenate([layer_1_out, layer_113_out, layer_114_out], axis=-2)
		layer_112_out = layer_112(layer_112_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_112 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
				new_list_of_inputs.append(i_2)
		else:
...
try:
	try:
		layer_111_concat = tf.keras.layers.concatenate([layer_1_out, layer_112_out, layer_113_out, layer_114_out])
		layer_111_out = layer_111(layer_111_concat)
	except ValueError:
		layer_111_concat = tf.keras.layers.concatenate([layer_1_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_111_out = layer_111(layer_111_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_111 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
				new_list_of_inputs.append(i_2)
		else:
...
try:
	try:
		layer_1005_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1005_out = layer_1005(layer_1005_concat)
	except ValueError:
		layer_1005_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1005_out = layer_1005(layer_1005_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1005 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
				new_list_of_inputs.append(i_2)
		else:
	...
	layer_1005_concat = strong_concat_1005(new_list_of_inputs)
	layer_1005_out = layer_1005(layer_1005_concat)
if layer_1005.count_params() != 0:
	if 1005 in set_weigths:
		if len(layer_1005.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1005, set_weigths[1005])]+layer_1005.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1005, set_weigths[1005])]
		layer_1005.set_weights(new_w)


try:
	count = 1
	for s in layer_1005_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1005:',count)
except AttributeError:
	for i in layer_1005_out:
		count = 1
		for s in layer_1005_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1006_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1006_out = layer_1006(layer_1006_concat)
	except ValueError:
		layer_1006_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1006_out = layer_1006(layer_1006_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1006 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1006_concat = strong_concat_1006(new_list_of_inputs)
	layer_1006_out = layer_1006(layer_1006_concat)
if layer_1006.count_params() != 0:
	if 1006 in set_weigths:
		if len(layer_1006.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1006, set_weigths[1006])]+layer_1006.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1006, set_weigths[1006])]
		layer_1006.set_weights(new_w)


try:
	count = 1
	for s in layer_1006_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1006:',count)
except AttributeError:
	for i in layer_1006_out:
		count = 1
		for s in layer_1006_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1007_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1007_out = layer_1007(layer_1007_concat)
	except ValueError:
		layer_1007_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1007_out = layer_1007(layer_1007_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1007 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1007_concat = strong_concat_1007(new_list_of_inputs)
	layer_1007_out = layer_1007(layer_1007_concat)
if layer_1007.count_params() != 0:
	if 1007 in set_weigths:
		if len(layer_1007.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1007, set_weigths[1007])]+layer_1007.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1007, set_weigths[1007])]
		layer_1007.set_weights(new_w)


try:
	count = 1
	for s in layer_1007_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1007:',count)
except AttributeError:
	for i in layer_1007_out:
		count = 1
		for s in layer_1007_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1008_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1008_out = layer_1008(layer_1008_concat)
	except ValueError:
		layer_1008_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1008_out = layer_1008(layer_1008_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1008 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1008_concat = strong_concat_1008(new_list_of_inputs)
	layer_1008_out = layer_1008(layer_1008_concat)
if layer_1008.count_params() != 0:
	if 1008 in set_weigths:
		if len(layer_1008.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1008, set_weigths[1008])]+layer_1008.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1008, set_weigths[1008])]
		layer_1008.set_weights(new_w)


try:
	count = 1
	for s in layer_1008_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1008:',count)
except AttributeError:
	for i in layer_1008_out:
		count = 1
		for s in layer_1008_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1009_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1009_out = layer_1009(layer_1009_concat)
	except ValueError:
		layer_1009_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1009_out = layer_1009(layer_1009_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1009 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1009_concat = strong_concat_1009(new_list_of_inputs)
	layer_1009_out = layer_1009(layer_1009_concat)
if layer_1009.count_params() != 0:
	if 1009 in set_weigths:
		if len(layer_1009.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1009, set_weigths[1009])]+layer_1009.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1009, set_weigths[1009])]
		layer_1009.set_weights(new_w)


try:
	count = 1
	for s in layer_1009_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1009:',count)
except AttributeError:
	for i in layer_1009_out:
		count = 1
		for s in layer_1009_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1010_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1010_out = layer_1010(layer_1010_concat)
	except ValueError:
		layer_1010_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1010_out = layer_1010(layer_1010_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1010 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1010_concat = strong_concat_1010(new_list_of_inputs)
	layer_1010_out = layer_1010(layer_1010_concat)
if layer_1010.count_params() != 0:
	if 1010 in set_weigths:
		if len(layer_1010.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1010, set_weigths[1010])]+layer_1010.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1010, set_weigths[1010])]
		layer_1010.set_weights(new_w)


try:
	count = 1
	for s in layer_1010_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1010:',count)
except AttributeError:
	for i in layer_1010_out:
		count = 1
		for s in layer_1010_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1011_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1011_out = layer_1011(layer_1011_concat)
	except ValueError:
		layer_1011_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1011_out = layer_1011(layer_1011_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1011 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1011_concat = strong_concat_1011(new_list_of_inputs)
	layer_1011_out = layer_1011(layer_1011_concat)
if layer_1011.count_params() != 0:
	if 1011 in set_weigths:
		if len(layer_1011.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1011, set_weigths[1011])]+layer_1011.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1011, set_weigths[1011])]
		layer_1011.set_weights(new_w)


try:
	count = 1
	for s in layer_1011_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1011:',count)
except AttributeError:
	for i in layer_1011_out:
		count = 1
		for s in layer_1011_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_1012_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_1012_out = layer_1012(layer_1012_concat)
	except ValueError:
		layer_1012_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_1012_out = layer_1012(layer_1012_concat)
except ValueError:
	#print('do strong concat!')
	strong_concat_1012 = Strong_concat()
	new_list_of_inputs = []
	for i in [layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out]:
		if type(i) == tuple:
			for i_2 in i:
	...
	layer_1012_concat = strong_concat_1012(new_list_of_inputs)
	layer_1012_out = layer_1012(layer_1012_concat)
if layer_1012.count_params() != 0:
	if 1012 in set_weigths:
		if len(layer_1012.get_weights()) > 1:
			new_w = [get_new_weigths(layer_1012, set_weigths[1012])]+layer_1012.get_weights()[1:]
		else:
			new_w = [get_new_weigths(layer_1012, set_weigths[1012])]
		layer_1012.set_weights(new_w)


try:
	count = 1
	for s in layer_1012_out.shape:
		count = count*s
		if count > 10000000:
			raise NameError
	#print('do #1012:',count)
except AttributeError:
	for i in layer_1012_out:
		count = 1
		for s in layer_1012_out.shape:
			count = count*s
			if count > 10000000:
...
try:
	try:
		layer_0_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_0_out = layer_0(layer_0_concat)
	except ValueError:
		layer_0_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_0_out = layer_0(layer_0_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_16_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_16_out = layer_16(layer_16_concat)
	except ValueError:
		layer_16_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_16_out = layer_16(layer_16_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_17_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_17_out = layer_17(layer_17_concat)
	except ValueError:
		layer_17_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_17_out = layer_17(layer_17_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_18_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_18_out = layer_18(layer_18_concat)
	except ValueError:
		layer_18_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_18_out = layer_18(layer_18_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_19_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_19_out = layer_19(layer_19_concat)
	except ValueError:
		layer_19_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_19_out = layer_19(layer_19_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_20_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_20_out = layer_20(layer_20_concat)
	except ValueError:
		layer_20_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_20_out = layer_20(layer_20_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_21_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_21_out = layer_21(layer_21_concat)
	except ValueError:
		layer_21_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_21_out = layer_21(layer_21_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_22_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_22_out = layer_22(layer_22_concat)
	except ValueError:
		layer_22_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_22_out = layer_22(layer_22_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_23_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_23_out = layer_23(layer_23_concat)
	except ValueError:
		layer_23_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_23_out = layer_23(layer_23_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_24_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_24_out = layer_24(layer_24_concat)
	except ValueError:
		layer_24_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_24_out = layer_24(layer_24_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_25_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_25_out = layer_25(layer_25_concat)
	except ValueError:
		layer_25_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_25_out = layer_25(layer_25_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_26_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_26_out = layer_26(layer_26_concat)
	except ValueError:
		layer_26_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_26_out = layer_26(layer_26_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_27_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_27_out = layer_27(layer_27_concat)
	except ValueError:
		layer_27_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_27_out = layer_27(layer_27_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_28_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_28_out = layer_28(layer_28_concat)
	except ValueError:
		layer_28_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_28_out = layer_28(layer_28_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_29_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_29_out = layer_29(layer_29_concat)
	except ValueError:
		layer_29_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_29_out = layer_29(layer_29_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_30_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_30_out = layer_30(layer_30_concat)
	except ValueError:
		layer_30_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_30_out = layer_30(layer_30_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_31_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_31_out = layer_31(layer_31_concat)
	except ValueError:
		layer_31_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_31_out = layer_31(layer_31_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_32_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_32_out = layer_32(layer_32_concat)
	except ValueError:
		layer_32_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_32_out = layer_32(layer_32_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_33_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_33_out = layer_33(layer_33_concat)
	except ValueError:
		layer_33_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_33_out = layer_33(layer_33_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_34_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_34_out = layer_34(layer_34_concat)
	except ValueError:
		layer_34_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_34_out = layer_34(layer_34_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_35_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_35_out = layer_35(layer_35_concat)
	except ValueError:
		layer_35_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_35_out = layer_35(layer_35_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_36_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_36_out = layer_36(layer_36_concat)
	except ValueError:
		layer_36_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_36_out = layer_36(layer_36_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_37_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_37_out = layer_37(layer_37_concat)
	except ValueError:
		layer_37_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_37_out = layer_37(layer_37_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_38_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_38_out = layer_38(layer_38_concat)
	except ValueError:
		layer_38_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_38_out = layer_38(layer_38_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_39_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_39_out = layer_39(layer_39_concat)
	except ValueError:
		layer_39_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_39_out = layer_39(layer_39_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_40_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_40_out = layer_40(layer_40_concat)
	except ValueError:
		layer_40_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_40_out = layer_40(layer_40_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_41_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_41_out = layer_41(layer_41_concat)
	except ValueError:
		layer_41_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_41_out = layer_41(layer_41_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_42_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_42_out = layer_42(layer_42_concat)
	except ValueError:
		layer_42_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_42_out = layer_42(layer_42_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_43_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_43_out = layer_43(layer_43_concat)
	except ValueError:
		layer_43_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_43_out = layer_43(layer_43_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_44_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_44_out = layer_44(layer_44_concat)
	except ValueError:
		layer_44_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_44_out = layer_44(layer_44_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_45_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_45_out = layer_45(layer_45_concat)
	except ValueError:
		layer_45_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_45_out = layer_45(layer_45_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_46_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_46_out = layer_46(layer_46_concat)
	except ValueError:
		layer_46_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_46_out = layer_46(layer_46_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_47_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_47_out = layer_47(layer_47_concat)
	except ValueError:
		layer_47_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_47_out = layer_47(layer_47_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_48_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_48_out = layer_48(layer_48_concat)
	except ValueError:
		layer_48_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_48_out = layer_48(layer_48_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_49_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_49_out = layer_49(layer_49_concat)
	except ValueError:
		layer_49_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_49_out = layer_49(layer_49_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_50_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_50_out = layer_50(layer_50_concat)
	except ValueError:
		layer_50_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_50_out = layer_50(layer_50_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_51_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_51_out = layer_51(layer_51_concat)
	except ValueError:
		layer_51_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_51_out = layer_51(layer_51_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_52_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_52_out = layer_52(layer_52_concat)
	except ValueError:
		layer_52_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_52_out = layer_52(layer_52_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_53_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_53_out = layer_53(layer_53_concat)
	except ValueError:
		layer_53_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_53_out = layer_53(layer_53_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_54_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_54_out = layer_54(layer_54_concat)
	except ValueError:
		layer_54_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_54_out = layer_54(layer_54_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_55_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_55_out = layer_55(layer_55_concat)
	except ValueError:
		layer_55_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_55_out = layer_55(layer_55_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_56_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_56_out = layer_56(layer_56_concat)
	except ValueError:
		layer_56_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_56_out = layer_56(layer_56_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_57_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_57_out = layer_57(layer_57_concat)
	except ValueError:
		layer_57_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_57_out = layer_57(layer_57_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_58_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_58_out = layer_58(layer_58_concat)
	except ValueError:
		layer_58_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_58_out = layer_58(layer_58_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_59_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_59_out = layer_59(layer_59_concat)
	except ValueError:
		layer_59_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_59_out = layer_59(layer_59_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_60_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_60_out = layer_60(layer_60_concat)
	except ValueError:
		layer_60_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_60_out = layer_60(layer_60_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_61_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_61_out = layer_61(layer_61_concat)
	except ValueError:
		layer_61_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_61_out = layer_61(layer_61_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_62_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_62_out = layer_62(layer_62_concat)
	except ValueError:
		layer_62_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_62_out = layer_62(layer_62_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_63_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_63_out = layer_63(layer_63_concat)
	except ValueError:
		layer_63_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_63_out = layer_63(layer_63_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_64_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_64_out = layer_64(layer_64_concat)
	except ValueError:
		layer_64_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_64_out = layer_64(layer_64_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_65_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_65_out = layer_65(layer_65_concat)
	except ValueError:
		layer_65_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_65_out = layer_65(layer_65_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_66_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_66_out = layer_66(layer_66_concat)
	except ValueError:
		layer_66_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_66_out = layer_66(layer_66_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_67_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_67_out = layer_67(layer_67_concat)
	except ValueError:
		layer_67_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_67_out = layer_67(layer_67_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_68_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_68_out = layer_68(layer_68_concat)
	except ValueError:
		layer_68_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_68_out = layer_68(layer_68_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_69_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_69_out = layer_69(layer_69_concat)
	except ValueError:
		layer_69_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_69_out = layer_69(layer_69_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_70_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_70_out = layer_70(layer_70_concat)
	except ValueError:
		layer_70_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_70_out = layer_70(layer_70_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_71_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_71_out = layer_71(layer_71_concat)
	except ValueError:
		layer_71_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_71_out = layer_71(layer_71_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_72_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_72_out = layer_72(layer_72_concat)
	except ValueError:
		layer_72_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_72_out = layer_72(layer_72_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_73_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_73_out = layer_73(layer_73_concat)
	except ValueError:
		layer_73_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_73_out = layer_73(layer_73_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_74_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_74_out = layer_74(layer_74_concat)
	except ValueError:
		layer_74_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_74_out = layer_74(layer_74_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_75_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_75_out = layer_75(layer_75_concat)
	except ValueError:
		layer_75_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_75_out = layer_75(layer_75_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_76_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_76_out = layer_76(layer_76_concat)
	except ValueError:
		layer_76_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_76_out = layer_76(layer_76_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_77_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_77_out = layer_77(layer_77_concat)
	except ValueError:
		layer_77_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_77_out = layer_77(layer_77_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_78_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_78_out = layer_78(layer_78_concat)
	except ValueError:
		layer_78_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_78_out = layer_78(layer_78_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_79_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_79_out = layer_79(layer_79_concat)
	except ValueError:
		layer_79_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_79_out = layer_79(layer_79_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_80_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_80_out = layer_80(layer_80_concat)
	except ValueError:
		layer_80_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_80_out = layer_80(layer_80_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_81_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_81_out = layer_81(layer_81_concat)
	except ValueError:
		layer_81_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_81_out = layer_81(layer_81_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_82_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_82_out = layer_82(layer_82_concat)
	except ValueError:
		layer_82_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_82_out = layer_82(layer_82_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_83_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_83_out = layer_83(layer_83_concat)
	except ValueError:
		layer_83_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_83_out = layer_83(layer_83_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_84_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_84_out = layer_84(layer_84_concat)
	except ValueError:
		layer_84_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_84_out = layer_84(layer_84_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_85_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_85_out = layer_85(layer_85_concat)
	except ValueError:
		layer_85_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_85_out = layer_85(layer_85_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_86_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_86_out = layer_86(layer_86_concat)
	except ValueError:
		layer_86_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_86_out = layer_86(layer_86_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_87_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_87_out = layer_87(layer_87_concat)
	except ValueError:
		layer_87_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_87_out = layer_87(layer_87_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_88_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_88_out = layer_88(layer_88_concat)
	except ValueError:
		layer_88_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_88_out = layer_88(layer_88_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_89_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_89_out = layer_89(layer_89_concat)
	except ValueError:
		layer_89_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_89_out = layer_89(layer_89_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_90_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_90_out = layer_90(layer_90_concat)
	except ValueError:
		layer_90_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_90_out = layer_90(layer_90_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_91_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_91_out = layer_91(layer_91_concat)
	except ValueError:
		layer_91_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_91_out = layer_91(layer_91_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_92_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_92_out = layer_92(layer_92_concat)
	except ValueError:
		layer_92_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_92_out = layer_92(layer_92_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_93_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_93_out = layer_93(layer_93_concat)
	except ValueError:
		layer_93_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_93_out = layer_93(layer_93_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_94_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_94_out = layer_94(layer_94_concat)
	except ValueError:
		layer_94_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_94_out = layer_94(layer_94_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_95_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_95_out = layer_95(layer_95_concat)
	except ValueError:
		layer_95_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_95_out = layer_95(layer_95_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_96_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_96_out = layer_96(layer_96_concat)
	except ValueError:
		layer_96_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_96_out = layer_96(layer_96_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_97_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_97_out = layer_97(layer_97_concat)
	except ValueError:
		layer_97_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_97_out = layer_97(layer_97_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_98_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_98_out = layer_98(layer_98_concat)
	except ValueError:
		layer_98_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_98_out = layer_98(layer_98_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_99_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_99_out = layer_99(layer_99_concat)
	except ValueError:
		layer_99_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_99_out = layer_99(layer_99_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_100_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_100_out = layer_100(layer_100_concat)
	except ValueError:
		layer_100_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_100_out = layer_100(layer_100_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_101_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_101_out = layer_101(layer_101_concat)
	except ValueError:
		layer_101_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_101_out = layer_101(layer_101_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_102_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_102_out = layer_102(layer_102_concat)
	except ValueError:
		layer_102_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_102_out = layer_102(layer_102_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_103_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_103_out = layer_103(layer_103_concat)
	except ValueError:
		layer_103_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_103_out = layer_103(layer_103_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_104_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_104_out = layer_104(layer_104_concat)
	except ValueError:
		layer_104_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_104_out = layer_104(layer_104_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_105_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_105_out = layer_105(layer_105_concat)
	except ValueError:
		layer_105_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_105_out = layer_105(layer_105_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_106_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_106_out = layer_106(layer_106_concat)
	except ValueError:
		layer_106_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_106_out = layer_106(layer_106_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_107_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_107_out = layer_107(layer_107_concat)
	except ValueError:
		layer_107_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_107_out = layer_107(layer_107_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_108_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_108_out = layer_108(layer_108_concat)
	except ValueError:
		layer_108_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_108_out = layer_108(layer_108_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_109_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_109_out = layer_109(layer_109_concat)
	except ValueError:
		layer_109_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_109_out = layer_109(layer_109_concat)
except ValueError:
	#print('do strong concat!')
...
try:
	try:
		layer_110_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out])
		layer_110_out = layer_110(layer_110_concat)
	except ValueError:
		layer_110_concat = tf.keras.layers.concatenate([layer_1_out, layer_111_out, layer_112_out, layer_113_out, layer_114_out], axis=-2)
		layer_110_out = layer_110(layer_110_concat)
except ValueError:
	#print('do strong concat!')
